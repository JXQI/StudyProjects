...
"""
def judge(box,boxes_pre,masks_pre):
    ...
    return [boxes_pre,masks_pre],status
"""
function: 清空队列
...
def clear_quene(quene):
    ...
    if ATTENTION_LOG:print("需要清空的队列{}".format(len(quene)))
    while(len(quene)>0):
        data=quene.pop(0)
        image_gray=data["nii_data"]
        mask_gray=np.zeros(list(image_gray.size))
        boxes_pre=data["boxes_pre"]
        masks_pre=data["masks_pre"]
        for index,item in enumerate(boxes_pre):
            i,j=item[0],item[1]
            ...
            if j>=N_SLICES:  #当检测框相交次数超过N的时候才画框保留，否则不画框
                ...
                # 为mask着色
                mask_gray+=masks_pre[index][0]
                print("\n标记位置{}\n".format(i))
        output.append(np.array(image_gray))
        mask_output.append(np.array(mask_gray))
    if ATTENTION_LOG:print("队列已经清空{}".format(len(QUENE)))
"""
...
"""
def attention(image_array,boxes_masks_pre,number,order):
    global QUENE
    global output
    global mask_output
    boxes_pre=boxes_masks_pre[0]
    masks_pre=[i.cpu()[0].tolist() for i in boxes_masks_pre[1]]
    boxes_pre = [list(i) for i in list(boxes_pre)]
    if boxes_pre:  #是否检测到有交集的boxes，如果没有直接清空队列，直接返回
        # 将数据转化为队列的数据类型
        N = [0] * len(boxes_pre)
        data = {"nii_data": image_array, "boxes_pre": [list(i) for i in zip(boxes_pre, N)],\
                "masks_pre":[list(i) for i in zip(masks_pre, N)]}  #元组不能修改，这里需要转换成list
        if ATTENTION_LOG:print("需要检测的boxes{}".format(boxes_pre))
        if QUENE:
            if ATTENTION_LOG:print("队列不为空{}".format(len(QUENE)))
            #判断是否于队列中的boxes有相交
            status=False
            for i in QUENE:  #更新了所有的boxes相交次数
                for index,j in enumerate(i["boxes_pre"]):
                    res,res_status=judge(j[0],data["boxes_pre"],data["masks_pre"])#如果相交返回True   #TODO:注意这里相交的话对当前的box数量也得加1
                    data["boxes_pre"] = res[0]  # 更新新添加的boxes相交次数
                    data["masks_pre"] = res[1]
                    if res_status:
                        j[1]+=1     #相交的次数+1
                        i["masks_pre"][index][1]+=1
                        status=True #有相交的boxes
                    else: #不相交判断当前box次数是否大于N,否则移除
                        if j[1]<N_SLICES:
                            i["boxes_pre"].remove(j)
                            # print(i["masks_pre"][index])
                            i["masks_pre"].remove(i["masks_pre"][index])
                        if ATTENTION_LOG:print("boxes判定结果不相交")
            if not status:#如果不相交，需要清空队列，保存新的队列
                if ATTENTION_LOG:print("没有相交的boxes")
                clear_quene(QUENE)
        QUENE.append(data)
        if ATTENTION_LOG:print("队列添加新的队列{}".format(len(QUENE)))
    else:
        ...
        mask_gray = np.zeros(list(image_array.size))
        mask_output.append(np.array(mask_gray))
    if order==number:clear_quene(QUENE)
